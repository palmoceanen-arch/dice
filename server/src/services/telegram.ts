...
const pendingPurchases = new Map<string, { oderId: number; itemId: number; priceStars: number }>();
...
export async function handlePreCheckoutQuery(preCheckoutQuery: {
  ...
  const itemResult = await query<{ id: number; price_stars: number }>(
    'SELECT id, price_stars FROM item_catalog WHERE id = $1',
    [purchase.itemId]
  );
  ...
  const ownershipResult = await query(
    ...
    [purchase.oderId, purchase.itemId]
  );
...
export async function handleSuccessfulPayment(
...
): Promise<{ success: boolean; itemId?: number; oderId?: number }> {
  const { invoice_payload, telegram_payment_charge_id } = payment;
  
  // Check for duplicate payment (idempotency)
  const existingPayment = await query(
    'SELECT id FROM purchases WHERE telegram_payment_id = $1',
  ...
  try {
    ...
    const insertResult = await query(
      `INSERT INTO purchases (user_id, item_id, telegram_payment_id, stars_amount, status, created_at) 
       VALUES ($1, $2, $3, $4, 'completed', NOW())
       ON CONFLICT (telegram_payment_id) DO NOTHING
       RETURNING id`,
      [purchase.oderId, purchase.itemId, telegram_payment_charge_id, purchase.priceStars]
    );
...
export function getPendingPurchase(payload: string) {
