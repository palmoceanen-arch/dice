 ...
 */
export function xpFromGame(won: boolean, totalPlayers: number): number {
...
export async function getPlayerStats(userId: number): Promise<PlayerStats | null> {
  const result = await query<{
    ...
    `SELECT id, xp, level, games_played, wins, losses, pips
 ...
 * broadcasts.
 */
export async function recordGameResult(args: RecordGameResultArgs): Promise<PlayerStats | null> {
  const { userId, won, totalPlayers } = args;
  if (totalPlayers < MIN_PLAYERS_FOR_STATS) return null;
  ...
  try {
    const result = await query<{
      ...
      `UPDATE users
       ...
       WHERE id = $1
       RETURNING id, xp, level, games_played, wins, losses, pips`,
      [userId, gained, won ? 1 : 0, won ? 0 : 1],
    );
