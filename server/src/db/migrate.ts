...
function generateDefaultItems(): string[] {
  const items: string[] = [];
...
async function migrate() {
  ...
  try {
    ...
    const starterDiceResult = await pool.query<{ id: number; code: string }>(
      'SELECT id, code FROM item_catalog WHERE code = ANY($1) AND type = \'dice\'',
      [starterDiceCodes]
    );
    
    if (starterDiceResult.rows.length > 0) {
      // Get all users
      const usersResult = await pool.query<{ id: number }>('SELECT id FROM users');
