 ...
 */
export function checkRateLimit(identifier: string, messageType: string): RateLimitResult {
 ...
 */
export function resetRateLimit(identifier: string, messageType?: string): void {
  if (messageType) {
 ...
 */
export function getRateLimitStats(): { totalTracked: number; blockedCount: number } {
  let totalTracked = 0;
  let blockedCount = 0;
  const now = Date.now();
