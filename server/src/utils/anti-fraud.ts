...
interface RollHistory {
  timestamps: number[];
  rollData: { timestamp: number; pips: number }[]; // Track pips per roll
  suspiciousCount: number;
}

// Track roll history per user
const userRollHistory = new Map<number, RollHistory>();
...
export function getFraudStats() {
  ...
  for (const history of userRollHistory.values()) {
    if (history.suspiciousCount > 0) stats.suspiciousUsers++;
    if (history.suspiciousCount >= 5) stats.flaggedUsers++;
  }
 ...
 */
export function resetFraudCounter(userId: number) {
