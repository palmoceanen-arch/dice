...
async function cleanupInactive() {
    ...
    try {
        ...
        const preview = await pool.query(`SELECT COUNT(*) as count, 
       ...
       WHERE last_online < NOW() - INTERVAL '${DAYS_INACTIVE} days'`);
        ...
        const rollsDeleted = await pool.query(`DELETE FROM rolls 
       WHERE lobby_id IN (
        ...
        // Delete bets
        const betsDeleted = await pool.query(`DELETE FROM bets 
       WHERE session_id IN (
        ...
        const sessionsDeleted = await pool.query(`DELETE FROM game_sessions 
       WHERE lobby_id IN (
         SELECT id FROM lobbies 
         WHERE created_at < NOW() - INTERVAL '7 days' 
         OR status = 'finished'
        ...
        const playersDeleted = await pool.query(`DELETE FROM lobby_players 
       WHERE lobby_id IN (
         SELECT id FROM lobbies 
         WHERE created_at < NOW() - INTERVAL '7 days' 
         OR status = 'finished'
        ...
        const lobbiesDeleted = await pool.query(`DELETE FROM lobbies 
       WHERE created_at < NOW() - INTERVAL '7 days'
       OR status = 'finished'
       OR host_id IN (
         SELECT id FROM users 
        ...
        const invitationsDeleted = await pool.query(`DELETE FROM invitations 
...
cleanupInactive();
//# sourceMappingURL=cleanup-inactive.js.map