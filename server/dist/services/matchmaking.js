...
const DEFAULT_GAME_MODE = 'poker_dice';
...
export const ALLOWED_BETS = [0, 10, 50, 100, 500];
...
function queueSizeForMode(mode) {
    let n = 0;
...
function sweep() {
    ...
    for (const [, entries] of anyBuckets) {
        if (entries.length < ANY_MIN_PLAYERS)
            continue;
        ...
        for (let i = 1; i < entries.length && compatible.length < ANY_MAX_PLAYERS; i++) {
            ...
            if (isCompatible(seed, cand, now))
                compatible.push(cand);
        }
        if (compatible.length < ANY_MIN_PLAYERS)
            continue;
        // Stamp the "min reached at" marker so we can wait for extras.
        for (const e of compatible) {
            if (e.minReachedAt == null)
                e.minReachedAt = now;
        }
        const groupMinReached = Math.min(...compatible.map((e) => e.minReachedAt ?? now));
        const sealNow = compatible.length >= ANY_MAX_PLAYERS ||
...
function expireTimedOutEntries(now) {
    for (const entry of queue.values()) {
...
async function chargeAllBets(lobbyId, userIds, amount) {
    const failed = [];
...
async function safeLeaveLobby(lobbyId, userId) {
    try {
...
async function autoStartMatch(lobbyId, userIds, gameMode) {
    try {
...
async function pickTableId(userId) {
    // 1. host's equipped table
    const user = await getUserById(userId);
    if (user?.equippedTableId != null)
        return user.equippedTableId;
    // 2. any available table in the catalog (deterministic by id)
    const result = await query(`SELECT id FROM item_catalog WHERE type = 'table' AND is_available = true
     ORDER BY id LIMIT 1`);
 ...
 */
export function markReady(userId) {
    ...
    if (!pending.userIds.includes(userId))
        return;
    if (pending.readyIds.has(userId))
        return;
 ...
 */
async function finalizePendingMatch(lobbyId) {
    const pending = pendingMatches.get(lobbyId);
    if (!pending || pending.finalized)
        return;
 ...
 */
async function cancelPendingMatch(pending, reason, unreadyIds) {
    pending.finalized = true;
    clearTimeout(pending.timer);
