...
export async function getActiveBoosts(userId) {
    const result = await pool.query(`SELECT * FROM user_boosts 
     WHERE user_id = $1 AND expires_at > NOW()
     ORDER BY activated_at DESC`, [userId]);
 ...
 */
export async function getBoostState(userId, boostId) {
    const result = await pool.query(`SELECT * FROM user_boosts 
     WHERE user_id = $1 AND boost_id = $2
     ORDER BY activated_at DESC
     LIMIT 1`, [userId, boostId]);
 ...
 */
export async function isBoostAvailable(userId, boostId) {
 ...
 * Activate a boost
 */
export async function activateBoost(userId, boostId, boostType, selectedParity) {
    ...
    try {
        ...
        await pool.query(`INSERT INTO user_boosts (user_id, boost_id, boost_type, activated_at, expires_at, available_at, selected_parity)
       ...
       DO UPDATE SET 
         ...
         expires_at = $5,
         available_at = $6,
         selected_parity = $7`, [userId, boostId, boostType, now, expiresAt, availableAt, selectedParity || null]);
 ...
 */
export async function calculatePipsMultiplier(userId, dice1, dice2) {
    const activeBoosts = await getActiveBoosts(userId);
    let multiplier = 1;
    let bonus = 0;
...
export async function cleanupExpiredBoosts() {
    const result = await pool.query(`DELETE FROM user_boosts 
