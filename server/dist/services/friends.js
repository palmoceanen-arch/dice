...
export async function sendFriendRequest(fromUserId, toUserId) {
    ...
    const existingFriend = await query('SELECT 1 FROM friends WHERE user_id = $1 AND friend_id = $2', [fromUserId, toUserId]);
    if (existingFriend.rows.length > 0)
        return { success: false };
    // Check if request already exists
    const existingRequest = await query(`SELECT id FROM friend_requests WHERE from_user_id = $1 AND to_user_id = $2 AND status = 'pending'`, [fromUserId, toUserId]);
    if (existingRequest.rows.length > 0)
        return { success: false };
    // Check if reverse request exists (they already sent us a request)
    const reverseRequest = await query(`SELECT id FROM friend_requests WHERE from_user_id = $1 AND to_user_id = $2 AND status = 'pending'`, [toUserId, fromUserId]);
    ...
    const result = await query(`INSERT INTO friend_requests (from_user_id, to_user_id) VALUES ($1, $2) RETURNING id`, [fromUserId, toUserId]);
    return { success: true, requestId: result.rows[0].id };
}
// Get pending friend requests for a user
export async function getPendingFriendRequests(userId) {
    const result = await query(`SELECT fr.id, fr.from_user_id, fr.to_user_id, fr.status, fr.created_at,
...
export async function acceptFriendRequest(requestId, userId) {
    ...
    const request = await query(`SELECT from_user_id, to_user_id FROM friend_requests WHERE id = $1 AND status = 'pending'`, [requestId]);
    ...
    await query(`INSERT INTO friends (user_id, friend_id, source) VALUES ($1, $2, 'search') ON CONFLICT DO NOTHING`, [userId, fromUserId]);
    await query(`INSERT INTO friends (user_id, friend_id, source) VALUES ($1, $2, 'search') ON CONFLICT DO NOTHING`, [fromUserId, userId]);
    return { success: true, fromUserId };
}
// Decline friend request
export async function declineFriendRequest(requestId, userId) {
    const result = await query(`UPDATE friend_requests SET status = 'declined' WHERE id = $1 AND to_user_id = $2 AND status = 'pending'`, [requestId, userId]);
    return (result.rowCount ?? 0) > 0;
}
// Legacy addFriend - now sends a request instead
export async function addFriend(userId, friendId, source = 'search') {
    const result = await sendFriendRequest(userId, friendId);
    return result.success;
}
export async function removeFriend(userId, friendId) {
    const result = await query('DELETE FROM friends WHERE (user_id = $1 AND friend_id = $2) OR (user_id = $2 AND friend_id = $1)', [userId, friendId]);
    return (result.rowCount ?? 0) > 0;
}
export async function areFriends(userId, friendId) {
    const result = await query(`SELECT 1 FROM friends WHERE user_id = $1 AND friend_id = $2 AND status = 'active'`, [userId, friendId]);
