...
const lobbyBettingStates = new Map();
// Rate limiting для защиты от спама
const betRateLimiter = new Map();
export class BettingManager {
    /**
     * Инициализация системы ставок для лобби
     */
    static initialize(lobbyId, minBet = 10) {
        const state = {
            bets: new Map(),
     ...
     * Валидация ставки
     */
    static async validateBet(userId, amount, lobbyId) {
        // Rate limit
        ...
        // Проверка баланса
        try {
            const result = await pool.query('SELECT pips FROM users WHERE id = $1', [userId]);
     ...
     */
    static async placeBet(userId, amount, lobbyId) {
        // Валидация
     ...
     */
    static async confirmBet(userId, lobbyId) {
        const state = lobbyBettingStates.get(lobbyId);
        if (!state) {
        ...
        try {
            ...
            // Списание с баланса
            const result = await client.query('UPDATE users SET pips = pips - $1 WHERE id = $2 AND pips >= $1 RETURNING pips', [amount, userId]);
            if (result.rows.length === 0) {
                throw new Error('Insufficient balance');
            }
            // Запись в БД
            await client.query('INSERT INTO game_bets (lobby_id, user_id, amount, status) VALUES ($1, $2, $3, $4)', [lobbyId, userId, amount, 'pending']);
     ...
     */
    static cancelBet(userId, lobbyId) {
        const state = lobbyBettingStates.get(lobbyId);
        if (!state)
            return false;
        if (state.confirmedPlayers.has(userId)) {
     ...
     */
    static async activateBets(lobbyId) {
        const state = lobbyBettingStates.get(lobbyId);
        if (!state)
            return false;
        try {
            await pool.query('UPDATE game_bets SET status = $1 WHERE lobby_id = $2 AND status = $3', ['active', lobbyId, 'pending']);
     ...
     */
    static async resolve(lobbyId, winners) {
        const state = lobbyBettingStates.get(lobbyId);
        if (!state) {
        ...
        try {
            ...
            if (winners.length === 0) {
                ...
                for (const [userId, amount] of state.bets) {
                    await client.query('UPDATE users SET pips = pips + $1 WHERE id = $2', [amount, userId]);
                    payouts.set(userId, amount);
                }
                await client.query('UPDATE game_bets SET status = $1, resolved_at = NOW() WHERE lobby_id = $2 AND status = $3', ['returned', lobbyId, 'active']);
                ...
                for (const winnerId of winners) {
                    await client.query('UPDATE users SET pips = pips + $1 WHERE id = $2', [winnerShare, winnerId]);
                    payouts.set(winnerId, winnerShare);
                    await client.query('UPDATE game_bets SET status = $1, resolved_at = NOW() WHERE lobby_id = $2 AND user_id = $3 AND status = $4', ['won', lobbyId, winnerId, 'active']);
                }
                // Проигравшие
                const losers = Array.from(state.bets.keys()).filter(id => !winners.includes(id));
                for (const loserId of losers) {
                    await client.query('UPDATE game_bets SET status = $1, resolved_at = NOW() WHERE lobby_id = $2 AND user_id = $3 AND status = $4', ['lost', lobbyId, loserId, 'active']);
                }
     ...
     */
    static async refundBet(userId, lobbyId) {
        const state = lobbyBettingStates.get(lobbyId);
        if (!state)
            return false;
        const amount = state.bets.get(userId);
        ...
        try {
            ...
            await client.query('UPDATE users SET pips = pips + $1 WHERE id = $2', [amount, userId]);
            // Обновление статуса
            await client.query('UPDATE game_bets SET status = $1, resolved_at = NOW() WHERE lobby_id = $2 AND user_id = $3 AND status = $4', ['returned', lobbyId, userId, 'pending']);
            await client.query('COMMIT');
            // Обновляем состояние
            state.bets.delete(userId);
     ...
     */
    static areAllPlayersReady(lobbyId, playerIds) {
        const state = lobbyBettingStates.get(lobbyId);
        if (!state)
            return false;
