...
export const MexicoMode = {
    ...
    processRoll(state, playerId, dice1, dice2) {
        const mexicoState = state.modeState;
        // Calculate score
 ...
 */
function getMexicoScoreName(dice1, dice2) {
    const high = Math.max(dice1, dice2);
    const low = Math.min(dice1, dice2);
    if (high === 2 && low === 1) {
        return 'MEXICO! 🇲🇽';
 ...
 */
function findNextPlayer(state, mexicoState, activePlayers) {
 ...
 */
function endRound(state, mexicoState, activePlayers, dice1, dice2, scoreName) {
    // Find lowest score(s)
