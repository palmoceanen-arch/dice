...
// === Admin ===
async function handleAdminGift(targetUserId, item) {
...
// === Profile ===
async function handleSetNickname(userId, nickname) {
...
async function handleGetShopItems(userId) {
    const items = await getShopItems();
    connections.send(userId, { type: 'shop_items', items });
}
async function handlePurchaseItem(userId, itemId) {
    // Get item details with config
    const itemResult = await query('SELECT id, name, description, price_stars, type, config FROM item_catalog WHERE id = $1', [itemId]);
    ...
    const ownershipResult = await query('SELECT 1 FROM user_items WHERE user_id = $1 AND item_id = $2', [userId, itemId]);
    ...
    // Create invoice link
    const result = await createStarsInvoice(userId, item.id, item.name, item.description || `Purchase ${item.name} in Street Dice`, item.price_stars);
...
// === Friends ===
async function handleSearchUser(userId, username) {
...
async function handleGetFriendRequests(userId) {
...
async function handleRespondFriendRequest(userId, requestId, accept) {
    if (accept) {
...
// === Referrals ===
async function handleGetReferralStats(userId) {
...
async function handleGetReferralList(userId) {
...
async function handleJoinLobby(userId, lobbyId, screenWidth, screenHeight) {
    ...
    if (success) {
        ...
        if (conn) {
            ...
            let equippedTableName = null;
            if (user.equippedTableId) {
                const tableResult = await query(`SELECT name FROM item_catalog WHERE id = $1 AND type = 'table'`, [user.equippedTableId]);
...
async function handleLeaveLobby(userId) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId) {
...
async function handleReconnectGame(userId, lobbyId) {
...
async function handleStartGame(userId) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId) {
        connections.send(userId, { type: 'error', message: 'Not in a lobby' });
        return;
    ...
    if (success) {
        ...
        if (updatedLobby?.gameMode === 'street_craps') {
            const crapsState = gameState?.modeState;
            modeData = {
            ...
            const mexicoState = gameState?.modeState;
            modeData = {
...
// === Invitations ===
async function handleInviteFriend(userId, friendId) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId) {
    ...
    if (invitation) {
        ...
        if (friendConn) {
            ...
            if (friend?.telegramId && senderConn?.user.nickname) {
                const sent = await sendGameInvite(friend.telegramId, senderConn.user.nickname, conn.lobbyId, lobbyData.gameMode);
                if (sent) {
...
async function handlePlayerReady(userId) {
    const conn = connections.getConnection(userId);
...
async function handleThrowStart(userId, throwPower, effectId, selectedDice) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame) {
...
async function handleThrowSound(userId, soundType, velocity, time) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame)
        return;
...
async function handleThrowEnd(userId, finalResult) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame) {
        connections.send(userId, { type: 'error', message: 'Not in game' });
        return;
    ...
    if (lobbyStatus.gameMode === 'poker_dice') {
        ...
        if (result) {
            ...
            game.saveRoll(conn.lobbyId, userId, diceValues[0] || 0, diceValues[1] || 0, result.outcome).catch(err => logger.error('Failed to save roll', err));
            // Handle game over - get payouts first
            let payoutsObj;
            if (result.gameOver) {
                const payouts = await game.endGame(conn.lobbyId);
        ...
        game.saveRoll(conn.lobbyId, userId, dice1, dice2, crapsResult.outcome).catch(err => logger.error('Failed to save roll', err));
        ...
        if (result) {
            // Save roll with result (non-blocking - don't wait)
            game.saveRoll(conn.lobbyId, userId, dice1, dice2, result.outcome).catch(err => logger.error('Failed to save roll', err));
            // Handle game over - get payouts first
            let payoutsObj;
            if (result.gameOver) {
                const payouts = await game.endGame(conn.lobbyId);
        ...
        game.saveRoll(conn.lobbyId, userId, dice1, dice2).catch(err => logger.error('Failed to save roll', err));
...
async function handleThrowDiceSync(userId, throwData) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame) {
        connections.send(userId, { type: 'error', message: 'Not in game' });
        return;
...
async function handleRollDice(userId, dice1, dice2) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame) {
        connections.send(userId, { type: 'error', message: 'Not in game' });
        return;
...
async function handlePassTurn(userId) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame) {
        connections.send(userId, { type: 'error', message: 'Not in game' });
        return;
...
async function handleGreedyPigStop(userId) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame) {
        connections.send(userId, { type: 'error', message: 'Not in game' });
        return;
...
// === Palmo's Dice ===
async function handlePalmosTake(userId) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame) {
        connections.send(userId, { type: 'error', message: 'Not in game' });
        return;
...
async function handlePalmosReroll(userId, selectedDice) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame) {
        connections.send(userId, { type: 'error', message: 'Not in game' });
        return;
...
// === Helpers ===
function broadcastToLobby(lobbyId, message, excludeUserId) {
...
function broadcastTurnChange(lobbyId, playerId) {
...
async function notifyFriendsStatusChanged(userId, status) {
    const friends = await getFriends(userId);
    for (const friend of friends) {
        connections.send(friend.friendId, {
...
async function handleSoloRollComplete(userId, data) {
...
// === Boosts ===
async function handleGetBoostStates(userId) {
...
async function handleActivateBoost(userId, boostId, parity) {
...
// === Custom Dice ===
async function handleSaveCustomDice(userId, config) {
...
// === Betting ===
async function handlePlaceBet(userId, amount) {
    const conn = connections.getConnection(userId);
...
async function handleConfirmBet(userId) {
    const conn = connections.getConnection(userId);
    if (!conn || !conn.lobbyId) {
        connections.send(userId, { type: 'bet_error', message: 'Not in a lobby' });
        return;
...
async function handleCancelBet(userId) {
    const conn = connections.getConnection(userId);
    if (!conn || !conn.lobbyId) {
        connections.send(userId, { type: 'bet_error', message: 'Not in a lobby' });
        return;
...
// === Reactions ===
async function handleSendReaction(userId, content) {
    const conn = connections.getConnection(userId);
    if (!conn?.lobbyId || !conn.inGame) {
        connections.send(userId, { type: 'error', message: 'Not in game' });
        return;
